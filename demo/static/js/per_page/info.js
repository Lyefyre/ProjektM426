...
var checkImagesLoaded = function() {
  imagesLoadedCount++;
  if (isSuper && imagesLoadedCount === allImagesCount) {
    var elpos = document.getElementById(lastEdited);
    if (elpos) {
      elpos.scrollIntoView({
        block: "start",
        behavior: "smooth"
      });
    }
...
window.addEventListener('message', function(event) {
  if (event.origin === 'http://david.abotsi.com') {
    var adj = event.data;
    var sheet = (function() {
    ...
    for (var i = 0; i < adj.length; i++) {
      var s_ih = '';
      s_ih += adj[i].element_qs + '{' + adj[i].css + '}' + '\n';
      sheet.insertRule(s_ih);
    }
  }
}, false);
...
document.addEventListener('DOMContentLoaded', function() {
  /* style adjustments */
  var fixtop = document.getElementsByClassName('fixtop')[0];
  if (isSuper) {
    fixtop.classList.add('fixtopedit');
  }
  document.getElementsByClassName('divs-striped')[0].parentNode.classList.add('info');
  ...
  for (var i = 0; i < imagecontainers.length; i++) {
    var pk = imagecontainers[i].getAttribute('images-index');
    if (photos[pk]) {
      for (var j = 0; j < photos[pk].length; j++) {
        ...
        if (photos[pk][j].toLowerCase().indexOf('.jpg') != -1 || photos[pk][j].toLowerCase().indexOf('.jpeg') != -1 || photos[pk][j].toLowerCase().indexOf('.png') != -1) {
          ...
          imagecontainers[i].appendChild(makeContainerElement(useurl, aheight, 'img'));
          ...
          imagecontainers[i].appendChild(makeContainerElement(useurl, aheight, ext, text_dict[pk][j]));
